...
test('T-133 keyboard · a payment is opened and allocated without a mouse; money inputs carry screen-reader names', async ({ page }) => {
  ...
  await expect(first).toHaveValue('0.000')                        // the typed value stuck (no refetch reset it)
  const second = page.getByTestId('allocate-A11Y-2')
  for (let i = 0; i < 60 && !(await second.evaluate((el) => el === document.activeElement)); i++) await page.keyboard.press('Tab')
