...
describe('collection queue (slice 5 AC-17)', () => {
  ...
  it('shows the hard confirmation before escalating and sends the reason', async () => {
    ...
    const calls = stubFetch([() => detail, () => ({ items: [], totalCount: 0, today: '' }), () => ({ items: [], totalCount: 0 }), () => ({ caseId: 'a', allowSplitDunningDuringDispute: false, invoices: [] }), () => ({ items: [], totalCount: 0 }), () => ({ items: [] }), () => ({ ...item('a', 78, 'Alpha'), status: 'Escalated', automationDisabled: true }), () => ({ ...detail, case: { ...detail.case, status: 'Escalated', automationDisabled: true } }), () => ({ items: [], totalCount: 0, today: '' }), () => ({ items: [], totalCount: 0 }), () => ({ caseId: 'a', allowSplitDunningDuringDispute: false, invoices: [] }), () => ({ items: [], totalCount: 0 })])
