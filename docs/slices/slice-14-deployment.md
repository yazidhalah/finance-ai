...
| D-6 | The build stage pins `sdk:10.0.103` (the 10.0.1xx band) rather than `sdk:10.0` | `sdk:10.0` resolves to 10.0.4xx, which `global.json` (`latestPatch`) refuses — and whose `dotnet format` disagrees with 10.0.1xx on one construct. Keeping the feature band identical between developers, CI and the image keeps formatting deterministic. |
